...
def test_flow():
    ...
    r = requests.post(f"{BASE_URL}/login", data={"username": "admin@example.com", "password": "adminpassword"})
    ...
    r = requests.post(f"{BASE_URL}/login", data={"username": "student@example.com", "password": "studentpassword"})
    ...
    r = requests.post(f"{BASE_URL}/admin/jobs", json=job_data, headers=admin_headers)
    job_id = r.json().get("id")
    results.append({"endpoint": "POST /admin/jobs", "status": r.status_code, "response": r.json()})

    # 6. Student View Jobs
    print("Testing Student View Jobs...")
    r = requests.get(f"{BASE_URL}/student/jobs", headers=student_headers)
    ...
    with open("test_resume.pdf", "rb") as f:
        r = requests.post(f"{BASE_URL}/student/upload-resume", files={"file": f}, headers=student_headers)
    ...
    r = requests.get(f"{BASE_URL}/admin/jobs/{job_id}/applicants", headers=admin_headers)
    ...
    r = requests.get(f"{BASE_URL}/student/applications", headers=student_headers)
    ...
    with open("endpoints.md", "w") as f:
        ...
        f.write("| --- | --- | --- | --- |\n")
        for res in results:
            method = res["endpoint"].split(" ")[0]
            path = res["endpoint"].split(" ")[1]
